...
<head>
    ...
    <title>newyork.dev — Developer Jobs - video2 - last changed: aug 3 2026</title>
...
<body>
    ...
    mobileSidebar: false,
    expandedJobId: null,
    /* drives the sheet enter animation */
    sheetVisible: false,
    ...
    jobs: [
        {
            ...
            tags:['PHP','Laravel','Backend','Hybrid','Senior'], posted:'2d',
        ...
        },
        {
            ...
            tags:['DevOps','Kubernetes','Python','Senior'], posted:'4d',
    ...
    selectJob(id){
        ...
        if(!this.isDesktop){
            /* reset sheet video state for the new job */
            const j=this.jobs.find(j=>j.id===id);
            if(j) j._vidSheet=false;
    ...
    closeDetail(){
        if(!this.isDesktop){
            this.sheetVisible=false;
            /* wait for transition before clearing job so content doesn't snap away */
            setTimeout(()=>{ this.expandedJobId=null; document.body.style.overflow=''; },280);
        } else {
    ...
    filteredJobs(){
        ...
        if(this.activeSort==='hybrid')  list=list.sort((a,b)=>b.location.toLowerCase().includes('hybrid')-a.location.toLowerCase().includes('hybrid'));
        if(this.activeSort==='remote')  list=list.sort((a,b)=>b.location.toLowerCase().includes('remote')-a.location.toLowerCase().includes('remote'));
    ...
    navigate(url){ if(url&&url!=='#') window.location.href=url; },
