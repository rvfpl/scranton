...
<head>
    ...
    <style>
        :root {
            ...
            --accent-blue:#60a5fa;--accent-amber:#fbbf24;
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .job-card.featured{border-color:rgba(74,222,128,0.25)}
        ...
        .badge-new{padding:2px 7px;border-radius:4px;font-size:10px;font-weight:600;font-family:'IBM Plex Mono',monospace;background:var(--accent-dim);color:var(--accent);border:1px solid rgba(74,222,128,0.2)}
        .badge-remote{padding:2px 7px;border-radius:4px;font-size:10px;font-weight:600;font-family:'IBM Plex Mono',monospace;background:rgba(96,165,250,0.1);color:var(--accent-blue);border:1px solid rgba(96,165,250,0.2)}
        ...
        .post-btn{padding:7px 14px;border-radius:6px;font-size:13px;font-weight:600;background:var(--accent);color:#0a1a0f;border:none;cursor:pointer;transition:opacity .12s;font-family:'IBM Plex Sans',sans-serif}
...
<body>
<div x-data="{
    jobs:[],loading:true,fetchError:false,search:'',activeFilters:[],activeSort:'latest',
    savedJobs:JSON.parse(localStorage.getItem('savedJobs')||'[]'),mobileSidebar:false,
    specializations:[
        {key:'Backend',icon:'⬡'},{key:'Frontend',icon:'◈'},{key:'DevOps',icon:'⟳'},
        {key:'Senior',icon:'★'},{key:'React',icon:'⚛'},{key:'Laravel',icon:'▲'},
        {key:'PHP',icon:'⬡'},{key:'Remote',icon:'⌂'},{key:'Hybrid',icon:'⇌'}
    ],
    init(){
        fetch('/api/v1/jobs')
            .then(r=>{if(!r.ok)throw new Error(r.status);return r.json()})
            .then(({data})=>{this.jobs=data;this.loading=false})
            .catch(()=>{this.fetchError=true;this.loading=false})
    },
    toggleFilter(k){this.activeFilters.includes(k)?this.activeFilters=this.activeFilters.filter(f=>f!==k):this.activeFilters.push(k)},
    toggleSave(id){this.savedJobs.includes(id)?this.savedJobs=this.savedJobs.filter(j=>j!==id):this.savedJobs.push(id);localStorage.setItem('savedJobs',JSON.stringify(this.savedJobs))},
    isSaved(id){return this.savedJobs.includes(id)},
    filteredJobs(){
        let list=this.jobs.filter(job=>{
            const s=this.search.toLowerCase();
            const ms=!s||job.title.toLowerCase().includes(s)||job.company.toLowerCase().includes(s)||(job.tags||[]).some(t=>t.toLowerCase().includes(s));
            const mf=this.activeFilters.length===0||this.activeFilters.every(f=>(job.tags||[]).includes(f));
            return ms&&mf;
        });
    ...
    logoColor(c){const cols=['#1e3a5f','#1a3a2a','#3a1a2a','#2a2a1a','#1a2a3a','#2a1a3a'];let h=0;for(let ch of c)h=(h*31+ch.charCodeAt(0))&0xffffffff;return cols[Math.abs(h)%cols.length]}
}">
...
<nav class="topnav">
    <div class="flex items-center justify-between h-full px-4 gap-3">
        ...
        <div class="flex items-center gap-2">
            <a href="/post-job"><button class="post-btn hidden sm:block">+ Post Job</button></a>
...
<div class="flex">
    ...
    <main class="flex-1 min-w-0 p-4 md:p-6">
        <div class="flex items-center justify-between mb-5 flex-wrap gap-3">
            ...
            <div style="font-family:'IBM Plex Mono',monospace;font-size:12px;color:var(--text-muted)"><span x-text="filteredJobs().length"></span> offers</div>
        </div>

        <div x-show="activeFilters.length>0" class="flex flex-wrap gap-2 mb-4">
            <template x-for="f in activeFilters" :key="f">
                <span class="filter-pill active" @click="toggleFilter(f)"><span x-text="f"></span><span style="font-size:9px;margin-left:2px">✕</span></span>
            </template>
        </div>
        ...
        <div x-show="loading" class="flex flex-col gap-3">
            <template x-for="i in 4" :key="i">
                <div class="job-card"><div class="flex gap-4 items-start"><div class="skeleton" style="width:44px;height:44px;border-radius:8px;flex-shrink:0"></div><div class="flex-1 flex flex-col gap-2"><div class="skeleton" style="height:16px;width:60%"></div><div class="skeleton" style="height:12px;width:40%"></div><div class="flex gap-2 mt-1"><div class="skeleton" style="height:20px;width:60px;border-radius:4px"></div><div class="skeleton" style="height:20px;width:80px;border-radius:4px"></div></div></div></div></div>
            </template>
        </div>

        {{-- Error --}}
        <div x-show="fetchError" class="empty-state"><div style="font-size:32px;margin-bottom:12px">⚠</div><div style="font-size:14px">Failed to load jobs</div></div>

        {{-- Jobs --}}
        <div x-show="!loading&&!fetchError" class="flex flex-col gap-3">
            <template x-for="job in filteredJobs()" :key="job.id">
                <div class="job-card" :class="job.is_featured?'featured':''" @click="window.location.href=job.url">
                    <div class="flex gap-4 items-start">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex-1 min-w-0">
                                    <div class="flex flex-wrap items-center gap-2 mb-1">
                                        ...
                                        <span x-show="job.is_featured" class="badge-new">featured</span>
                                        <span x-show="job.location&&job.location.includes('Remote')" class="badge-remote">remote</span>
                                    </div>
                                    <div class="flex flex-wrap items-center" style="font-size:12px;color:var(--text-secondary)">
                                ...
                                <div class="flex items-center gap-2 flex-shrink-0">
                                    <span class="salary-text" x-show="job.salary&&job.salary!=='Salary not disclosed'" x-text="job.salary"></span>
                            ...
                            <div class="flex flex-wrap gap-1.5 mt-3" @click.stop>
                                <template x-for="tag in (job.tags||[]).slice(0,5)" :key="tag">
                                    <span class="tech-tag" :style="activeFilters.includes(tag)?'border-color:var(--accent);color:var(--accent);background:var(--accent-dim)':''" @click="toggleFilter(tag)" x-text="tag"></span>
                                </template>
                                <span x-show="(job.tags||[]).length>5" class="tech-tag" x-text="'+'+(job.tags.length-5)+' more'" style="cursor:default"></span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div x-show="filteredJobs().length===0&&!loading" class="empty-state">
    ...
    <footer class="border-t border-black/10 bg-[#121212]">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-2 py-2">
            <div class="flex flex-col lg:flex-row gap-10 lg:items-start lg:justify-between">
 


            <div class="mt-2 pt-2 border-t border-black/10 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between text-xs text-gray-500">
                <div>
